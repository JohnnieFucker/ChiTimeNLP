...
var TimeUnit = function () {
     ...
     *
     */
    function TimeUnit(expTime, isPreferFuture, timeBase) {
    ...
    _createClass(TimeUnit, [{
        key: '_checkContextTime',
        value: function _checkContextTime(checkTimeIndex) {
        ...
        key: '_preferFutureWeek',
        value: function _preferFutureWeek(weekday) {
            /** 1. 确认用户选项 */
            if (!this.isPreferFuture) {
                return;
            }
            /** 2. 检查被检查的时间级别之前，是否没有更高级的已经确定的时间，如果有，则不进行倾向处理. */
            var checkTimeIndex = 2;
            for (var i = 0; i < checkTimeIndex; i++) {
                if (this._tp.tunit[i] !== -1) return;
            }
        ...
        key: 'normSetYear',
        value: function normSetYear() {
            /** 假如只有两位数来表示年份 */
            var rule = new RegExp('[0-9]{2}(?=年)', 'g');
            var match = this.timeExpression.match(rule);
        ...
        key: 'normSetMonth',
        value: function normSetMonth() {
            var rule = new RegExp('((10)|(11)|(12)|([1-9]))(?=月)', 'g');
            var match = this.timeExpression.match(rule);
            if (match && match.length > 0) {
        ...
        key: 'normSetMonthFuzzyDay',
        value: function normSetMonthFuzzyDay() {
            var rule = new RegExp('((10)|(11)|(12)|([1-9]))[月|.|-]([0-2][0-9]|3[0-1]|[1-9])', 'g');
            var match = this.timeExpression.match(rule);
            if (match && match.length > 0) {
                var m = match[0];
                rule = new RegExp('[月|.|-]');
        ...
        key: 'normSetDay',
        value: function normSetDay() {
            var rule = new RegExp('([0-2][0-9]|3[0-1]|[1-9])(?=(日|号))', 'g');
            var match = this.timeExpression.match(rule);
            if (match && match.length > 0) {
        ...
        key: 'normSetHour',
        value: function normSetHour() {
            var tmp = this.timeExpression.replace(/(周|星期)[1-7]/g, '');
            var rule = new RegExp('([0-2]?[0-9])(?=(点|时))');
            var match = tmp.match(rule);
            if (match && match.length > 0) {
        ...
        key: 'normSetMinute',
        value: function normSetMinute() {
            var rule = new RegExp('([0-5]?[0-9](?=分(?!钟)))', 'g');
            var match = this.timeExpression.match(rule);
            if (match && match.length > 0) {
                ...
                var tmp = util.reverseStr(this.timeExpression);
        ...
        value: function normSetSecond() {
            var rule = new RegExp('([0-5]?[0-9](?=秒))', 'g');
            var match = this.timeExpression.match(rule);
            if (match && match.length > 0) {
        ...
        key: 'normSetTotal',
        value: function normSetTotal() {
            var tmpParser = [];
            var tmp = this.timeExpression.replace(/(周|星期)[1-7]/g, '');
            var rule = new RegExp('([0-2]?[0-9]):[0-5]?[0-9]:[0-5]?[0-9]', 'g');
            var match = tmp.match(rule);
            if (match && match.length > 0) {
        ...
        key: 'normSetBaseRelated',
        value: function normSetBaseRelated() {
            var d = this.timeBase;

            var flag = [false, false, false]; // 观察时间表达式是否因当前相关时间表达式而改变时间

            var rule = new RegExp('(\\d+)(?=天[以之]?前)', 'g');
            var match = this.timeExpression.match(rule);
            var day = 0;
            if (match && match.length > 0) {
                flag[2] = true;
        ...
        key: 'normSetCurRelated',
        value: function normSetCurRelated() {
            var d = this.timeBase;

            var flag = [false, false, false]; // 观察时间表达式是否因当前相关时间表达式而改变时间
            var rule = new RegExp('前年', 'g');
            var match = this.timeExpression.match(rule);
            var day = 0;
            if (match && match.length > 0) {
                flag[0] = true;
        ...
        key: 'timeNormalization',
        value: function timeNormalization() {
            this.normSetYear();
            this.normSetMonth();
            this.normSetDay();
